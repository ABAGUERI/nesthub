...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!code || !redirectUri) {
      return jsonResponse(400, { message: 'code et redirectUri requis' });
    }
    ...
    if (!clientId || !clientSecret) {
      return jsonResponse(500, { message: 'Google OAuth secrets manquants' });
    }
    ...
    if (!tokenResponse.ok) {
      ...
      return jsonResponse(tokenResponse.status, {
        message: 'Erreur échange OAuth Google',
        error: errorBody?.error ?? 'unknown_error',
        description: errorBody?.error_description ?? 'unknown_error',
      });
    }
    ...
    if (!gmailAddress) {
      return jsonResponse(400, { message: 'Email Google introuvable' });
    }
    ...
    if (upsertError) {
      ...
      return jsonResponse(500, { message: 'Erreur sauvegarde Google' });
    }
